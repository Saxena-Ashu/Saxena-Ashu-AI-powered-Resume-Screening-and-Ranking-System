...
def rank_resumes(job_desc, file_paths):
    ...
    for file_path in file_paths:
        ...
        resume_data = {
            ...
            "skill_match": f"{skill_score:.1f}%" if best_job else "N/A",
            "exp_match": f"{exp_score:.1f}%" if best_job else "N/A",
            "edu_match": f"{edu_score:.1f}%" if best_job else "N/A",
            "technical_skills": [
                {"name": s, "match": normalize_text(s) in resume_text} 
                for s in best_job["skills"]] if best_job else [],
            "relevant_experience": [
                {"detail": e, "match": normalize_text(e) in resume_text} 
                for e in best_job["experience"]] if best_job else [],
            "education_required": [
                {"detail": ed, "match": normalize_text(ed) in resume_text} 
                for ed in best_job["education"]] if best_job else [],
            "additional_info": extract_additional_sections(resume_text)
        }
